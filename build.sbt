...
lazy val atto = project.in(file("."))
  ...
  .dependsOn(core, tests, scalaz71, scalaz72, cats, cats06)
  .aggregate(core, tests, scalaz71, scalaz72, cats, cats06)
...
lazy val cats06 = project.in(file("compat/cats")).dependsOn(core)
  .settings(buildSettings ++ commonSettings ++ publishSettings)
  .settings(name := "atto-compat-cats06")
  .settings(target := file("compat/cats/target06"))
